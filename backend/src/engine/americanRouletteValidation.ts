...
/** Valid numbers: 0-36 and -1 (00) */
function isValidNumber(n: number): boolean {
  return n === DOUBLE_ZERO || (Number.isInteger(n) && n >= 0 && n <= 36);
}

function areAllValidNumbers(numbers: number[]): boolean {
  return numbers.length > 0 && numbers.every(isValidNumber);
}
...
/** Top line: 0, 00, 1, 2, 3 — unique to American roulette */
function isTopLine(numbers: number[]): boolean {
  if (numbers.length !== 5) return false;
  const sorted = [...numbers].sort((a, b) => a - b);
  return (
    sorted[0] === DOUBLE_ZERO &&
    sorted[1] === 0 &&
    sorted[2] === 1 &&
    sorted[3] === 2 &&
    sorted[4] === 3
...
function isEvenMoney(type: AmericanRouletteBetType, numbers: number[]): boolean {
...
export function validateAmericanRouletteBets(
  ...
  for (const bet of bets) {
    ...
    if (!areAllValidNumbers(bet.numbers)) {
      return { valid: false, error: 'Numbers must be integers between -1 (00) and 36' };
    }
    ...
    if (bet.amount < AMERICAN_ROULETTE_CONFIG.min_bet) {
      return {
        valid: false,
        error: `Bet amount ${bet.amount} below minimum ${AMERICAN_ROULETTE_CONFIG.min_bet}`,
      };
    }
    ...
    switch (bet.type) {
      ...
      case 'street':
        if (!isStreet(bet.numbers)) {
          return { valid: false, error: 'Street must be a 3-number column group' };
        }
        break;
      case 'corner':
        ...
        break;
      case 'topLine':
        if (!isTopLine(bet.numbers)) {
          return { valid: false, error: 'Top line must be [0, -1, 1, 2, 3] (0, 00, 1, 2, 3)' };
        }
        break;
      case 'sixLine':
  ...
  if (totalBet > AMERICAN_ROULETTE_CONFIG.max_total_bet) {
    return {
      valid: false,
      error: `Total bet ${totalBet} exceeds table limit ${AMERICAN_ROULETTE_CONFIG.max_total_bet}`,
    };
  }
