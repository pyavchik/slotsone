...
import { CATEGORIES, getGamesByCategory, getAllGames } from '@/data/catalog';
...
export default function LobbyPage() {
  ...
  const categoryCounts = useMemo(() => {
    ...
    for (const cat of CATEGORIES) {
      counts[cat.value] = getGamesByCategory(cat.value).length;
    }
    return counts;
  }, []);

  const getFilteredGames = (category: GameCategory) => {
    let games = getGamesByCategory(category);
