...
test.describe('CV Landing – actions bar', { tag: ['@smoke', '@regression'] }, () => {
  ...
  test('action bar items appear in correct order', async ({ page }) => {
    ...
    await expect(items.nth(1)).toHaveText('java automation');
    await expect(items.nth(2)).toHaveText('python automation');
    await expect(items.nth(3)).toHaveText('requirements');
    await expect(items.nth(4)).toHaveText('test cases');
    await expect(items.nth(5)).toHaveText('postman');
    await expect(items.nth(6)).toHaveText('swager');
    await expect(items.nth(7)).toHaveText('sql');
    await expect(items.nth(8)).toHaveText('test design');
    await expect(items.nth(9)).toHaveText('bug report');
    await expect(items.nth(10)).toHaveText('allure report');
    await expect(items.nth(11)).toHaveText('jmeter RTP');
    await expect(items.nth(12)).toHaveText('jmeter load test');
    await expect(items.nth(13)).toHaveText('game math');
    await expect(items.nth(14)).toHaveText('exploratory testing');
    await expect(items.nth(15)).toHaveText('QA strategy');
    await expect(items.nth(16)).toHaveText('time machine SRS');
    await expect(items.nth(17)).toHaveText('time machine tests');
    await expect(items.nth(18)).toHaveText('i18n testing');
    await expect(items.nth(19)).toHaveText('responsive testing');
    await expect(items.nth(20)).toHaveText('visual regression');
    await expect(items.nth(21)).toHaveText('network testing');
    await expect(items.nth(22)).toHaveText('GraphQL testing');
    await expect(items.nth(23)).toHaveText('playwright typescript');
